...
from newutils import get_exclude_dicts, get_to_exclude, predict_t5, predict_bert, clean_preds_t5, clean_preds_ssm, get_template, load_jsonl, flatten, partial_match, is_x_before_y, get_paraphrased_templates

def run(model_name, relation_name, batch_size, seed):
    ...
    # from pararel
    paraphrased_templates = get_paraphrased_templates(RELATION)

    if len(paraphrased_templates) > 1:
        template = random.choice(paraphrased_templates)
        print('randomly selected a template!')
    else:
        print('one or zero templates, no variance!')
        return

    
    x_before_y = is_x_before_y(template)
    ...
    if len(df) == 0:
        ...
        with open('results_para.csv', 'a') as f:
            f.write('{}, {}, {}, {}, {}\n'.format(MODEL_NAME, RELATION, SEED, len(df), 'NA', 'NA'))
    ...
    if x_before_y:
        ...
        inputs11 = df[subject].apply(lambda x: input_text.format(MASK_TOKEN, x)).tolist()
        inputs21 = df[object].apply(lambda x: input_text.format(x, MASK_TOKEN)).tolist()
    ...
    if x_before_y:
        ...
        inputs12 = pd.Series(preds11).apply(lambda x: input_text.format(x, MASK_TOKEN)).tolist()
        inputs22 = pd.Series(preds21).apply(lambda x: input_text.format(MASK_TOKEN, x)).tolist()
    ...
    results.to_csv('./results_para/results_{}_{}_{}.csv'.format(MODEL_NAME.replace('/', '_'), RELATION, SEED), index=False)
    
    with open('results_para.csv', 'a') as f:
        f.write('{}, {}, {}, {}, {}, {}\n'.format(MODEL_NAME, RELATION, SEED, len(df), np.mean(scores1+scores2), np.mean(np.bitwise_and(scores1, scores2))))
...
def main():
    ...
    run(args.model, args.relation, args.bs, args.seed)
